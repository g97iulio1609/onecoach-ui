...
const GlassCardComponent = forwardRef<HTMLDivElement, GlassCardProps>(
  (
    { className, intensity = 'medium', variant = 'default', gradient = false, children, ...props },
    ref
  ) => {
    ...
    const variantClasses = {
      ...
      active:
        'border-indigo-500/50 bg-indigo-50/30 dark:bg-indigo-900/20 dark:border-indigo-400/50',
    ...
    return (
      <div
        ...
        className={cn(
          ...
          'hover:-translate-y-1 hover:shadow-lg hover:shadow-neutral-200/50 dark:hover:border-neutral-700 dark:hover:shadow-neutral-900/50',
          className
        )}
        {...props}
      >
        ...
        {gradient && (
          <>
            {/* Subtle gradient border via background origin trick or absolute inset */}
            <div className="absolute inset-0 -z-10 bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 opacity-10" />
